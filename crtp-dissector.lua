...
-- FIXME: Define 'local f_crtp_AE483_m1 = ProtoField.uint8("crtp.AE483_m1", "m1")'
...
-- FIXME: Register the custom AE483 fields
...
function get_crtp_port_channel_names(port, channel)
	...
	if port == Ports.Console and channel == 0 then
		port_name = "Console"
	elseif port == Ports.Parameters then
	...
	elseif port == Ports.Commander then
		port_name = "Commander"
	elseif port == Ports.AE483 then
		port_name = "AE483"
		channel_name = "Default AE483 channel"
	elseif port == Ports.Memory then
		port_name = "Memory"
		if channel == 0 then
	...
	elseif port == Ports.Logging then
		port_name = "Logging"
		if channel == 0 then
			channel_name = "Table Of Contents"
		elseif channel == 1 then
	...
	elseif port == Ports.Localization then
	...
	elseif port == Ports.Commander_Generic then
		port_name = "Commander Generic"
	elseif port == Ports.Setpoint_Highlevel then
		port_name = "Setpoint Highlevel"
	elseif port == Ports.Platform then
		port_name = "Platform"
		if channel == 0 then
	...
	elseif port == Ports.LinkControl then
...
function handle_parameter_port(tree, receive, buffer, channel, size)
	...
	if (channel == 1 or channel == 2) and size > 2 then
		...
		-- CHANGED: item is now local variable. Will this work?
		local item = param_toc[var_id]
